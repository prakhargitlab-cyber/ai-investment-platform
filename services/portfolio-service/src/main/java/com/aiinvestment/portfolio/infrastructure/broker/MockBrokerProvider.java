...
public class MockBrokerProvider implements BrokerProvider {
    ...
    public List<BrokerAccount> fetchAccounts(UUID userId) {
        ...
        return List.of(
                new BrokerAccount("MOCK_EU_" + suffix, userId, BrokerType.MOCK, "mock-eu-account", "Mock EU Account", "EUR", BrokerAccountStatus.ACTIVE),
                new BrokerAccount("MOCK_INDIA_" + suffix, userId, BrokerType.MOCK, "mock-india-account", "Mock India Account", "INR", BrokerAccountStatus.ACTIVE)
        );
    }
    ...
    public List<BrokerPosition> fetchPositions(BrokerAccount account) {
        ...
        if (account.brokerAccountId().startsWith("MOCK_EU_")) {
            return List.of(
        ...
        if (account.brokerAccountId().startsWith("MOCK_INDIA_")) {
            return List.of(
    ...
    private static Instrument instrument(String ticker, String isin, String exchange, String mic, String companyName,
        ...
        return new Instrument(instrumentId, BrokerType.MOCK.name(), isin, isin, ticker, exchange, mic, companyName,
                AssetType.EQUITY, country, currency, sector, industry);
    }
}
